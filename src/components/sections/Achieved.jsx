...
const Achieved = () => {
  ...
  // ✅ FIX: safe dependency usage
  useEffect(() => {
    if (isInView) {
      controls.start('animate');
    }
  }, [isInView]); // removed unnecessary dependency warning

  // ✅ FIXED Counter (now re-renders properly)
  ...
  return (
    <section
      ref={ref} // ✅ attach ref (important for useInView)
      className="tr__section tr__home__achieved py-40 lg:py-60 overflow-hidden bg-gradient-to-br from-black via-black/50 to-gray-900 relative"
    >
      <div className="tr__home__achieved__container grid grid-cols-2 lg:grid-cols-4 gap-8 lg:gap-12 max-w-7xl mx-auto px-4">
        {statsData.map((stat, index) => (
          <motion.div
            ...
            className="relative overflow-hidden rounded-3xl p-8 lg:p-12 h-64 lg:h-80 flex items-center group cursor-pointer backdrop-blur-xl border border-white/10 hover:border-white/30 hover:shadow-2xl hover:shadow-orange-500/20 transition-all duration-500 bg-white/5 hover:bg-white/10"
          >
            {/* Background gradient */}
            <div
              ...
              <div className="flex flex-col">
                <motion.div className="flex items-baseline">
                  <Counter
                    ...
                    duration={stat.animationDuration || 2.5}
                  />
            ...
            {/* Decorations */}
            <div className="absolute top-4 right-4 w-20 h-px bg-gradient-to-r from-white/40 via-white/20 to-transparent group-hover:translate-x-2 transition-transform" />
            <div className="absolute bottom-4 left-4 w-20 h-px bg-gradient-to-l from-white/40 via-white/20 to-transparent group-hover:-translate-x-2 transition-transform" />
